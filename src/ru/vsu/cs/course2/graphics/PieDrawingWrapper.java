...
public class PieDrawingWrapper implements PieDrawer {
    ...
    @Override
    public void drawPie(int x1, int y1, int x2, int y2, double startAngle, double endAngle) {
        ...
        ellipseDrawer.drawArc(x1, y1, x2, y2, startAngle, endAngle);

        double val = Math.abs((startAngle - endAngle) % (Math.PI * 2));
        if (val >= EPS && val <= Math.PI * 2 - EPS) {
            lineDrawer.drawLine(xc, yc, xc + xs, yc + ys);
            lineDrawer.drawLine(xc, yc, xc + xe, yc + ye);
        }
    }
}
